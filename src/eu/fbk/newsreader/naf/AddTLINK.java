...
public class AddTLINK {
	...
	public static String[][] add_candidate_pairs(String [][] lines, int nbCol, HashMap<String,Integer> intCol, boolean allEventTmx0Pairs){
			...
			for (int i=sentences[j][0]; i<=sentences[j][1]; i++){
				if(k<=list_event.size() && lines[i][0] != null && lines[i][colEventId] != null && lines[i][colEventId].startsWith("e")){
					...
					if(((lines[i][colPOS].startsWith("V") || lines[i][colChunk].endsWith("VP")) 
							&& ! lines[i][colTense].matches("((INFINITIVE)|(PRESPART)|(PASTPART))\\+.*")) || allEventTmx0Pairs){
	...
	private static Boolean eventCloserTimex(String[][] lines, Integer[] sentences, String timex, String event){
		...
		for(int i=sentences[0]; i<sentences[1]; i++){
			...
			else if(lines[i][colEventId] != null && lines[i][colEventId].startsWith("e") && evPos > 0 && txPos == 0){
				return false;
			}
