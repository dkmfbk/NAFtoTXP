...
public class TXPtoNAF_v4 {
	...
	public static void TXP2NAF (File f, String [][] lines, String beginTimeSpan, String eltName) throws IOException{
		...
		if(eltName.equals("TIMEX3")){
			...
			for(int i=0; i<lines.length; i++){
				...
				if(lines[i][0] != null && lines[i].length > 10 && lines[i][7] != null && !lines[i][0].startsWith("# ")){
					if(lines[i][9] != null && lines[i][9].startsWith("tmx") && lines[i][7].startsWith("B-")){
						...
						if(!value.equals("")){
							...
							if(lines[i][11] != null && !lines[i][11].equals("_NULL_") && !lines[i][11].equals("null")&& !lines[i][11].equals("-")){
								tx.setAnchorTimeId(lines[i][11]);
							}

							if(lines[i][12] != null && !lines[i][12].equals("_NULL_") && !lines[i][12].equals("null")){
								if (listTimexId.containsKey(lines[i][12])){
									tx.setBeginPoint(listTimexId.get(lines[i][12]));
								}
							}
	
							if(lines[i][13] != null && !lines[i][13].equals("_NULL_") && !lines[i][13].equals("null")){
	...
	private static String getReverseRelation (String relType){
		if(relType.equals("BEFORE")) return "AFTER";
		else if(relType.equals("AFTER")) return "BEFORE";
		else if(relType.equals("IS_INCLUDED")) return "INCLUDES";
		else if(relType.equals("INCLUDES")) return "IS_INCLUDED";
		else if(relType.equals("BEGUN_BY")) return "BEGINS";
		else if(relType.equals("BEGINS")) return "BEGUN_BY";
		else if(relType.equals("ENDED_BY")) return "ENDS";
		else if(relType.equals("ENDS")) return "ENDED_BY";
		else if(relType.equals("IAFTER")) return "IBEFORE";
		else if(relType.equals("IBEFORE")) return "IAFTER";
		else if(relType.equals("SIMULTANEOUS")) return "SIMULTANEOUS";
		return "NONE";
	}
